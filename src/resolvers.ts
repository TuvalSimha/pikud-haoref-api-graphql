...
import { Alert, OrderBy, Resolvers, QueryAllAlertsFromTodayArgs, QueryAllAlertsByDateRangeArgs } from './resolvers-types';
	...
	Query: {
		allAlertsFromToday: async (_: any, args: QueryAllAlertsFromTodayArgs) => {
			...
			if (args.orderBy === OrderBy.CreatedAtAsc) {
				const sorted = alerts.sort((a, b) => new Date(a.alertDate).getTime() - new Date(b.alertDate).getTime());
				return sorted;
			}
			if (args.orderBy === OrderBy.CreatedAtDesc) {
				const sorted = alerts.sort((a, b) => new Date(b.alertDate).getTime() - new Date(a.alertDate).getTime());
				return sorted;
			}
		...
		allAlertsFromLastWeek: async (_: any, args: QueryAllAlertsFromTodayArgs) => {
		...
		allAlertsFromLastMonth: async (_: any, args: QueryAllAlertsFromTodayArgs) => {
		...
		allAlertsByDateRange: async (_: any, args: QueryAllAlertsByDateRangeArgs) => {
			console.log(args);
